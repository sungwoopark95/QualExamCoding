...
(base) soungmunkim@gimseongmun-ui-MacBookPro String % gcc 49_GroupAnagrams.c -o test
...
char*** anagram(char** s, int len_s, int* num_groups){
    ...
    char*** ans = malloc(sizeof(char**) * len_s);
    for(int i = 0; i < len_s; i++) {
        ans[i] = malloc(sizeof(char*) * len_s);
        for(int j = 0; j < len_s; j++) {
            ans[i][j] = malloc(sizeof(char) * 100); // 각 문자열은 최대 길이 100으로 가정
        }
    }
