...
private:
    ...
    void siftUp(int idx) {  // 현재 요소를 위로 올려 힙 조건을 만족시키는 함수
        ...
        while (idx > 0 && heap[idx] < heap[parentIdx]) {  // 현재 노드 값이 부모보다 작은 동안
    ...
    void siftDown(int idx) {  // 현재 요소를 아래로 내려 힙 조건을 만족시키는 함수
        ...
        while (childIdx < heap.size()) {  // 자식 노드가 존재하는 동안
            if (childIdx + 1 < heap.size() && heap[childIdx] > heap[childIdx + 1]) {  // 오른쪽 자식이 존재하고, 왼쪽 자식보다 값이 작은 경우
                childIdx++;  // 오른쪽 자식으로 설정
            }
            if (heap[childIdx] < heap[idx]) {  // 현재 노드 값보다 자식 노드 값이 작은 경우
...
int main() {  // 메인 함수
    MinHeap min_heap;  // 최소 힙 객체 생성
    min_heap.push(3);  // 값 추가
    min_heap.push(1);
    min_heap.push(2);
    std::cout << "Top element: " << min_heap.peek() << std::endl;  // 루트 값 출력
    std::cout << "Popped element: " << min_heap.pop() << std::endl;  // 루트 값 제거 후 출력
    std::cout << "Top element after pop: " << min_heap.peek() << std::endl;  // 제거 후 루트 값 출력
    return 0;
}
