...
void RBT::DeleteFixUp(Node* x) {
  ...
  while (x != root && x->color == COLOR_BLACK) {
    if (x == x->parent->left) {
      ...
      //case 1(sibling=Red) -> left rotate(x->parent) / color change(x->parent and w(x sibling) )
      ...
      //case 2(sibling은 black일 수 밖에 없고, sibling 자녀가 모두 black인 경우) sibling을 red로 바꾸고 parents에 대해서 조치
      if (w->left->color == COLOR_BLACK && w->right->color == COLOR_BLACK) {
        w->color = COLOR_RED;
        x = x->parent;
      } 

      // sibling 자식이 둘다 black은 아닐 경우(case3 or case4)
      else {

        //case 3(w의 왼쪽자식이 red, 오른쪽자식이 black인 경우 ) -> left rotate(w->parent) / color change(w(x sibling) and w->left) ->case4가 됨
