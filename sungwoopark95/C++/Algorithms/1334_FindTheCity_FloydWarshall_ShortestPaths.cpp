...
void PrintShortestPath(Graph* G, Vertex* start, int threshold) {
    ...
    // 5. 시작 정점에서 모든 정점까지의 최단 경로를 반복하여 출력합니다.
    for (auto& item : shortest_paths) {
        Vertex* to = item.first;   // 도착 정점
        vector<Vertex*> path = item.second;  // 시작에서 도착까지의 경로

        // 시작 정점과 도착 정점이 동일한 경우는 출력하지 않습니다.
        ...
        // 시작 정점에서 도착 정점까지의 거리가 threshold를 초과하면 출력하지 않습니다.
